...
主界面的“应用设置”集中管理运行范围、按键发送模式、分辨率、Gamma、Buff刷新阈值、安全格、最大重铸次数、助手自定义速度和主界面切换快捷键。具有常用预设的项目使用可编辑下拉框，既能直接选择，也能输入自定义值。主界面配置修改后会自动保存到全局设置和对应Profile；“保存”按钮与右下角托盘菜单仍可用于立即保存。默认按`Home`可在显示主界面和最小化到托盘之间切换，也可在“应用设置”中修改。右上角“—”会保存设置并最小化到托盘；点击“×”会询问是退出程序、最小化到托盘，还是取消关闭操作。
