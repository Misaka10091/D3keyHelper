...
发布tag必须使用四段数字版本，格式为`vMAJOR.MINOR.BUILD.REVISION`，例如`v1.4.2026.718`。推送tag后，GitHub Actions会将tag版本注入EXE的FileVersion和ProductVersion，生成同版本ZIP，并自动创建GitHub Release；Release同时包含EXE和ZIP。

```powershell
git tag v1.4.2026.718
git push origin v1.4.2026.718
```

本地可以用相同参数验证发布构建：

```powershell
.\scripts\build.cmd -Version v1.4.2026.718
```
...
- `dist/`：编译产物，不纳入版本控制。
- `d3oldsand.ini`和`profiles/`：本机运行配置，不纳入版本控制。
...
主界面的“应用设置”集中管理运行范围、按键发送模式、分辨率、Gamma、Buff刷新阈值、安全格、最大重铸次数和助手自定义速度。具有常用预设的项目使用可编辑下拉框，既能直接选择，也能输入自定义值。主界面的“保存”按钮会立即保存全局设置和所有Profile，右下角托盘菜单也提供相同入口。

Profile可以在应用设置中新增、重命名和删除。`d3oldsand.ini`只保存全局设置和Profile清单，每个Profile分别保存在`profiles/profile-001.ini`、`profiles/profile-002.ini`等独立文件中。旧版把多个Profile写在`d3oldsand.ini`不同区块中的配置仍可直接读取，并会在首次保存时自动迁移。
...
分享配置时，可以单独分享`profiles`目录中的某个Profile文件；全量迁移时同时复制`d3oldsand.ini`和`profiles`目录。
