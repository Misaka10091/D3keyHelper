...
默认产物为`dist\D3keyHelper.exe`和带文件版本号的ZIP，例如`dist\D3keyHelper-v1.4.2026.0718-windows-x64.zip`。如果AutoHotkey安装在自定义目录，可显式指定编译器：

```powershell
.\scripts\build.cmd -CompilerPath "D:\Tools\AutoHotkey\Compiler\Ahk2Exe.exe"
```

推送到`main`、创建Pull Request或手动运行GitHub Actions中的`Build EXE`工作流，也会生成带版本号的可下载构建产物。
