...
Profile可以在应用设置中新增、重命名和删除。程序启动时会自动加载`profiles/`目录下的全部INI文件，文件名就是Profile名称，不需要在全局配置中维护清单；目录为空时会自动创建`配置1.ini`。`settings.ini`只保存全局设置，`activatedprofile`记录当前Profile的文件名。
